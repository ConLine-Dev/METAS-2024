...
async function somarValoresAteDiaAtual(dados) {
  ...
  for (const item of dados) {
    ...
    const dataPagamento = new Date(
      dataPagamentoParts[0],
      dataPagamentoParts[1] - 1,
      dataPagamentoParts[2]
    );
...
async function total_valores_ate_dia_atual_ano_anterior(consulta) {
  ...
  const somaTotal = Object.values(resultadoSomaPorDia).reduce(
...
async function card_metas_anuais(fluxo_ano_anterior, fluxo_ano_atual) {
  ...
  const total_ano_anterior_ate_hoje =
    await total_valores_ate_dia_atual_ano_anterior(fluxo_ano_anterior);

  const meta_anual = Math.max(
    (total_ano_atual / (total_ano_anterior * meta)) * 100,
    0
  );
  const meta_hoje = Math.max(
    (total_ano_atual / (total_ano_anterior_ate_hoje * meta)) * 100,
    0
  );
...
async function grafico_financeiro_mes_mes(fluxo_ano_anterior, fluxo_ano_atual) {
  ...
  metas_mensais = await ajustarMetasComBaseEmResultadosAutomatico(
    metas_mensais,
    resultadosMensais
  );
  ...
  const valores_arrecadados = soma_mes_mes_atual.map(
    (item) => item.VALOR_CONVERTIDO_REAL
  );
...
async function soma_valores_modais(dados) {
  return dados.reduce((acumulador, item) => {
    ...
    acumulador[modalidade] =
      (acumulador[modalidade] || 0) + item.VALOR_CONVERTIDO_REAL;
    return acumulador;
  }, {});
}
...
async function grafico_modais(fluxo_ano_atual) {
  ...
  if (totalPorcentagem > 100) {
    ...
    for (const modalidade in porcentagens) {
      porcentagens[modalidade] = Number(
        (porcentagens[modalidade] / fatorAjuste).toFixed(2)
      );
    }
  }
